...
def proof_of_work(block):
    ...
    proof = 0
    # valid_proof = requests.post({'block_string': block_string, 'proof': proof})
    # validate = lambda x,y: requests.post({'block_string': x, 'proof': y})
    is_valid = valid_proof(block_string, proof)
    while not is_valid:
        proof += 1
        is_valid = valid_proof(block_string, proof)
